...
namespace ppm_fe.Helpers
{
    public class PdfHelper : BaseViewModel
    {
        ...
        // Generate PDF for Work
        public static async Task<PdfResult> GenerateBillingPdf(string destinationPath, BillingsInfoProfile billing)
        {
            var pdf = new PdfHelper();
            return await pdf.CreateBillingPdf(destinationPath, billing);
        }

        // Generate PDF for Billing
        public static async Task<string> GenerateWorkPdf(string title, Work work)
        {
                ...
                await Task.Run(() =>
                {
                    ...
                    if (isValidDate)
                    {
                        ...
                        Cell headingCellDatum = new Cell().Add(new Paragraph("Datum"))
                                         ...
                                         .SetBold() // Another way to make the text bold
                    ...
                    bodyTable.AddCell(headingCell).SetMinHeight(availableHeight);
                    ...
                    float fontSize = work?.Reflection?.Length > 200 ? 8f : 10f;

                    reflectionTable.AddCell(
                         new Cell()
                             .Add(new Paragraph(work?.Reflection)
                             .SetHyphenation(new HyphenationConfig("de", "DE", 3, 3)))
                             .SetFont(regularFont)
                    ...
                    // Kids Data Section
                    foreach (var kidData in work?.KidsData ?? Enumerable.Empty<WorkKidsData>())
                    {
                    ...
                    Cell additionalInfoCell = new Cell().Add(additionalInfoTable);
        ...
        public async Task<PdfResult> CreateBillingPdf(string dest, BillingsInfoProfile billing)
        {
            ...
            await Task.Run(() =>
            {
                try
                {
                    using (PdfDocument pdfDoc = new(new PdfWriter(dest)))
                    {
                        ...
                        // Add Date
                        var dateValue = billing.Date ?? string.Empty;
                        rightAlignedDiv.Add(CreateBorderedParagraph("Datum ", dateValue, regularFont, boldFont));
                        ...
                        // Add SteuerID 
                        var taxIdValue = billing.Steueridentifikationsnummer ?? string.Empty;
                        rightAlignedDiv.Add(CreateBorderedParagraph("Steueridentifkationsnummer", taxIdValue, regularFont, boldFont, true));
                        doc.Add(new Paragraph().SetMarginBottom(10));
                        ...
                        // Add Text for each BillingsDetails 
                        foreach (var detail in billing.BillingDetails ?? Enumerable.Empty<BillingDetail>())
                        {
                            doc.Add(new Paragraph()
                ...
                {
                    result.Properties = "Die PDF-Datei wird gerade von einer anderen Anwendung verwendet. Bitte schließen Sie alle Programme, die die Datei verwenden könnten, und versuchen Sie es erneut.";
                    LoggerHelper.LogError(GetType().Name, nameof(CreateBillingPdf), $"IOException occurred: {ex.Message}", new { dest, billing }, ex.StackTrace);
                }
                catch (Exception ex)
                {
                    result.Properties = $"Ein unerwarteter Fehler ist aufgetreten: {ex.Message}";
                    LoggerHelper.LogError(GetType().Name, nameof(CreateBillingPdf), $"Unexpected exception occurred: {ex.Message}", new { dest, billing }, ex.StackTrace);
                }
            });
        ...
        // Helper methode for create Billing PDF
